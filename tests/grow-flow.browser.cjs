...
(async () => {
  ...
  try {
    ...
    await page.route('https://*.supabase.co/**', async route => {
      ...
      else if(path.endsWith('/profiles')) data={role:'director_q'};
    ...
    async function openGrow() {
      await page.getByRole('button',{name:'Farmer tools',exact:true}).click();
      await page.getByRole('button',{name:'Farmer Grow · Beta',exact:true}).click();
    }
    ...
    await page.getByRole('button',{name:'Director Q',exact:true}).click();
    ...
    await planner.getByLabel('Crop width (ft)',{exact:true}).fill('2');
    await planner.getByLabel('Crop length (ft)',{exact:true}).fill('2');
    await planner.getByRole('button',{name:'Save crop & choose position',exact:true}).click();
    await planner.getByText('Could not save crop details.',{exact:false}).waitFor();
    await planner.getByRole('button',{name:'Save crop & choose position',exact:true}).click();
    await planner.getByText('Carrots details saved.',{exact:false}).waitFor();
    assert.equal(crops.filter(crop=>crop.crop==='Carrots').length,1,'retry does not duplicate crop');
    assert.equal(await planner.getByRole('button',{name:'Save this plot',exact:true}).isDisabled(),true,'must place before saving');
    await planner.getByRole('button',{name:'Column 1, row 1: empty plot square',exact:true}).click();
    ...
    await planner.getByRole('button',{name:'Choose position on plot',exact:true}).click();
    page.once('dialog',dialog=>dialog.accept());
    await planner.getByRole('button',{name:/^Column 1, row 1: Carrots/}).click();
    await planner.getByRole('button',{name:/^Column 1, row 1: Carrots/}).click();
    await planner.getByRole('group',{name:'Overlapping crops'}).getByRole('button',{name:/^Carrots/}).click();
    await planner.getByRole('button',{name:'Move this crop',exact:true}).click();
    await planner.getByRole('button',{name:'Column 3, row 1: empty plot square',exact:true}).click();
    ...
    await planner.getByText('Carrots details saved. Its position is unchanged.',{exact:true}).waitFor();
    failLayout=true;
    ...
    assert.equal(saved.document.layers[0].plotId,carrotId,'move keeps layer numbering');
    assert.equal(saved.document.layers[0].x,2);
    ...
    for(const width of [320,390,768,1024,1440]) {
      ...
      await planner.locator('.planner-mode').scrollIntoViewIfNeeded();
      await page.waitForTimeout(120);
      assert.equal(await page.evaluate(()=>document.documentElement.scrollWidth>innerWidth),false,'page overflow at '+width);
      const cell=await planner.locator('.planner-cell').first().boundingBox();assert.ok(cell.width>=48&&cell.height>=48);
    ...
    console.log('PASS: duplicate plot names, live draft labels, integrated crop retry/placement, overlap/move, stable numbering, edit dates, conflict recovery, linked harvest, reload, delete/cancel/restore, denied farm, and five viewport sizes.');

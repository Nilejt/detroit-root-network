...
const demos: Farm[] = [
  {
    ...
    slug: "qs-stall",
    blurb:
      "Fresh neighborhood produce, grown with care and shared with clarity.",
    test_tier: "T1",
    payment_methods: ["Cash", "Card"],
    selling_locations: [
      {
        id: "ql",
        location_name: "Eastern Market",
    ...
    volunteer_opportunities: [
      {
        id: "qv",
        title: "Saturday harvest help",
        description: "Help wash and bundle produce for market.",
        opportunity_date: "2026-09-19",
        volunteers_needed: 4,
        is_open: true,
      },
    ],
  ...
  {
    id: "o",
    name: "Oakland Avenue Farm",
    slug: "oakland-avenue",
    blurb: "Seasonal Detroit-grown produce and community food programs.",
    test_tier: "T1",
    payment_methods: ["Cash", "SNAP"],
    selling_locations: [
      {
        id: "ol",
        location_name: "North End farm stand",
  ...
  },
  {
    ...
    slug: "keep-growing-detroit",
    blurb:
      "Locally grown produce supporting a stronger food-sovereign Detroit.",
    test_tier: "T1",
    payment_methods: ["Cash", "Card", "SNAP"],
    selling_locations: [
      {
        id: "kl",
        location_name: "Grown in Detroit market",
  ...
  },
  {
    ...
    name: "D-Town Farm",
    slug: "d-town",
    blurb: "Community-rooted growing in Detroit's Rouge Park.",
    test_tier: "T2",
    payment_methods: ["Cash"],
    selling_locations: [
      {
        id: "dl",
        location_name: "Rouge Park",
  ...
  {
    id: "g",
    name: "Georgia Street Community Collective",
    slug: "georgia-street",
    blurb:
      "Community-grown food and neighborhood connection on Detroit's east side.",
    test_tier: "T2",
    payment_methods: ["Cash"],
    selling_locations: [
      {
        id: "gl",
        location_name: "Georgia Street",
        address: "8902 Vinton Ave",
        city: "Detroit",
        state: "MI",
        opens_at: "12:00",
        closes_at: "16:00",
...
export default function Home() {
  ...
  const [user, setUser] = useState<User | null>(null),
    ...
    [element, setElement] = useState("Discovery page");
  const load = async () => {
    if (!db) return;
    const { data } = await db
      .from("farms")
      .select(
        "*,selling_locations(*),inventory_items(*),volunteer_opportunities(*)",
      )
      .order("name");
  ...
  const saveNote = async (e: FormEvent) => {
    e.preventDefault();
    if (!db || !user)
      return setNotice("Sign in before leaving persistent feedback.");
    const { error } = await db.from("feedback").insert({
      author_id: user.id,
      page_path: `/${tab}`,
      element_label: element,
      element_key: element.toLowerCase().replaceAll(" ", "-"),
      comment,
    ...
    <main>
      ...
      {tab === "find" && (
        <section className="shell">
          <div className="intro">
            <div>
              <p className="eyebrow">OPEN AROUND DETROIT</p>
              <h1>Find fresh food near you</h1>
              <p>
                See what local farms have, where they are selling, and when to
                visit.
              </p>
            </div>
      ...
      {tab === "farmer" && (
        <Workspace
          ...
          sub="Designed for fast updates during busy market hours."
        >
          {!user ? (
      ...
      {tab === "feedback" && (
        <Workspace
          title="Product feedback, kept with the product"
          sub="Leave context-rich notes here; exports are now optional backups."
        >
          {!user ? (
            <Login {...{ email, setEmail, signIn }} />
          ) : (
            <div className="twocol">
              ...
              <div className="panel">
                <div className="row">
                  <div>
                    <h2>Feedback tally</h2>
                    <b>{notes.length} total</b>
                  </div>
                  <button
                    className="quiet"
                    type="button"
            ...
            {open.selling_locations.map((p) => (
              <div className="block" key={p.id}>
                ...
                <a
                  ...
                  target="_blank"
                  rel="noreferrer"
                  href={`https://www.openstreetmap.org/search?query=${encodeURIComponent(`${p.address}, ${p.city}, ${p.state}`)}`}
                >
                  Open in Maps
                </a>
              </div>
            ...
            {open.volunteer_opportunities.map((v) => (
              <div className="block" key={v.id}>
                ...
                <p>
                  ...
                  <br />
                  <small>
                    {v.opportunity_date} · {v.volunteers_needed} volunteers
                    needed
                  </small>
                </p>
              </div>

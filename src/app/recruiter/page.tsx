...
export const metadata: Metadata = {title:"Detroit Root Network Project Brief",robots:{index:false,follow:false}};
const messages: Record<string,string> = {
 ...
 unavailable:"Recruiter access is not available yet. Please contact the person who shared this page.",
};
export default async function RecruiterPage({searchParams}: {searchParams:Promise<{result?:string}>}) {
 ...
 if(!allowed) return <main className={styles.root}><div className={styles.gate}>
  ...
  <p className={styles.eyebrow}>PROJECT WALKTHROUGH</p>
  <h1>A closer look at the work</h1>
  <p>For recruiters, technical teams, and competition reviewers. Enter the five-digit code shared with you to explore the project brief.</p>
  <form action="/api/auth/recruiter" method="post">
   ...
   <button disabled={!config}>Open project brief <span aria-hidden="true">→</span></button>
  </form>
 ...
 return <main className={styles.root}>
  ...
  <div className={styles.hero}>
   <div><p className={styles.eyebrow}>DETROIT NEEDS · AGENTIC DEVELOPMENT COMPETITION</p><h1>From local food information to a working community platform</h1><p>{content.summary}</p>
  ...
  <nav className={styles.nav} aria-label="Project brief sections"><a href="#story">The story</a><a href="#stack">Architecture</a><a href="#delivery">Delivery</a><a href="#evidence">Evidence</a><a href="#next">Next steps</a></nav>
  <div className={styles.body}>
   ...
   <section id="next"><p className={styles.eyebrow}>05 / THE NEXT TEST</p><h2>Turn a working beta into measured community value</h2><ul>{content.next.map(n=><li key={n}>{n}</li>)}</ul><details><summary>A five-step competition demonstration</summary><ol>{content.demo.map(n=><li key={n}>{n}</li>)}</ol></details><p className={styles.notice}><strong>Before the larger demo</strong><br/>Remove temporary Owner and Director Q code login. The recruiter presentation gate is separate and never grants farm editing access.</p></section>
  </div><footer className={styles.footer}>Detroit Root Network · Technical project brief · September 2026</footer>
 </main>;
}
